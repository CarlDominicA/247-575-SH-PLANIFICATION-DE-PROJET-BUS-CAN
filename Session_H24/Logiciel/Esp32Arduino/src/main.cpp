...
void sendPotentiometerValue(int potValue) {
    char bufferPotValue[5]; // Buffer pour convertir un entier en chaîne avec une longueur fixe
    snprintf(bufferPotValue, sizeof(bufferPotValue), "%04d", potValue); // Convertir un int en string

    // Configure CAN message
    CanFrame potFrame = { 0 };
    potFrame.identifier = 0xB; // ID du message CAN
    potFrame.extd = 0; // 0 pour format standard (11 bits) et 1 pour format étendue (29 bits)
    potFrame.data_length_code = 8;

    // Copier chaque caractère de la chaîne dans les octets de données du message CAN
    for (int i = 0; i < 4; i++) {
...
void loop() {
  ...
  int intValue = static_cast<int>(valeurCanal); // Convertit valeurCanal en int
  delay(1000); //1 seconde avant la prochaine mesure
  sendPotentiometerValue(intValue); // envoi la valeur via CAN
}